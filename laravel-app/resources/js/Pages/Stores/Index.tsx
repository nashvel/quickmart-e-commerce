...
import { Search, Store, Tag, Star, MapPin, Clock, Filter, X, ArrowRight } from 'lucide-react';
...
const BANNER_ASSET_URL = '/storage/banners';

const StoreCard = forwardRef<HTMLDivElement, { store: Store }>(({ store }, ref) => {
    ...
    return (
        <div ref={ref} className="group bg-white rounded-2xl shadow-lg overflow-hidden transform hover:-translate-y-3 transition-all duration-300 ease-in-out hover:shadow-2xl">
            <Link href={`/stores/${store.id}/${store.name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`} className="block">
                <div className="h-36 bg-gradient-to-br from-blue-100 to-purple-100 relative overflow-hidden">
                    <img
                        ...
                        onError={(e) => {
                            ...
                            if (!target.dataset.errorHandled) {
                                ...
                                target.src = `https://placehold.co/400x150/E0E7FF/1D4ED8?text=Visit+${encodeURIComponent(store.name)}`;
                            }
                        }}
                    />
                    ...
                    <div className="absolute -bottom-12 left-4">
                        <div className="relative">
                            <img
                                src={store.logo ? `${LOGO_ASSET_URL}/${store.logo}` : getAvatarUrl(store.name)}
                                alt={`${store.name} logo`}
                                ...
                                onError={(e) => {
                                    ...
                                    if (!target.dataset.errorHandled) {
                                        target.dataset.errorHandled = 'true';
                                        target.src = getAvatarUrl(store.name);
                                    }
                                }}
                            />
                            <div className="absolute -bottom-1 -right-1 w-6 h-6 bg-primary rounded-full flex items-center justify-center">
                                <ArrowRight className="text-white text-xs transform group-hover:translate-x-0.5 transition-transform duration-300" size={12} />
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="pt-16 px-6 pb-6">
                    <div className="flex items-start justify-between mb-3">
                        <h3 className="text-xl font-bold text-gray-800 group-hover:text-primary transition-colors duration-300 line-clamp-1">
                            {store.name}
                        </h3>
                        <div className="flex items-center text-sm text-yellow-500 bg-yellow-50 px-2 py-1 rounded-lg">
                            <Star className="mr-1 fill-yellow-500" size={14} />
                    ...
                    <div className="space-y-2">
                        <div className="flex items-center text-sm text-gray-500">
                            <Tag className="mr-2 text-primary flex-shrink-0" size={16} />
                            <span className="truncate font-medium">{store.category || 'General Store'}</span>
                        </div>
                        <div className="flex items-center text-sm text-gray-500">
                            <MapPin className="mr-2 text-red-400 flex-shrink-0" size={16} />
                            <span className="truncate">{store.location || 'Available Online'}</span>
                        </div>
                        <div className="flex items-center text-sm text-gray-500">
                            <Clock className="mr-2 text-green-500 flex-shrink-0" size={16} />
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-100">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center text-primary font-semibold text-sm group-hover:text-primary-dark transition-colors">
                                <span className="mr-2">Explore</span>
                                <ArrowRight className="transform group-hover:translate-x-1 transition-transform duration-300" size={16} />
                            </div>
                        </div>
                    </div>
                </div>
...
export default function StoresIndex() {
    ...
    const fetchStores = async () => {
        try {
            const response = await axios.get('/api/stores?type=convenience');
                        ...
                        <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
                            <div className="w-full lg:w-2/5">
                                <div className="relative group">
                                    ...
                                    <input
                                        ...
                                        className="w-full py-3.5 pl-12 pr-12 border-2 border-gray-200 rounded-2xl bg-white focus:outline-none focus:ring-4 focus:ring-primary/20 focus:border-primary transition-all duration-300 text-gray-700 placeholder-gray-400"
                                    />
                                    {searchQuery && (
                                        <button
                            ...
                            <div className="flex items-center gap-4 w-full lg:w-auto">
                                <button
                                    ...
                                    className="lg:hidden flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-xl hover:bg-primary-dark transition-colors"
                                >
                                    <Filter className="text-sm" size={16} />
                                    <span>Filters</span>
                                </button>
                                ...
                                <select
                                    ...
                                    className="px-4 py-2 border-2 border-gray-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-gray-700"
                                >
                                    ...
                                    <div className="flex flex-wrap items-center gap-3">
                                        ...
                                        {categories.map(category => (
                                            <motion.button
                                                ...
                                                className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-300 whitespace-nowrap ${selectedCategory === category
                ...
                <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                    ...
                    {loading ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                            {[...Array(12)].map((_, index) => <StoreCardSkeleton key={index} />)}
                        </div>
