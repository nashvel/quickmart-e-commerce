...
export const LiveOrdersMap = ({ orders }: Props) => {
  ...
  const ordersWithDistance = ordersWithLocation.map(order => {
            ...
            {order.distance !== null && (
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1 text-xs text-blue-600 font-semibold">
                  <Navigation size={12} />
                  <span>{order.distance.toFixed(2)} km away</span>
                </div>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
      ...
      {selectedOrder && (
        <div className="mt-3 p-3 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-lg">
          ...
          <button
            ...
            className="mt-2 w-full bg-white text-blue-600 px-3 py-2 rounded-md text-sm font-semibold hover:bg-blue-50 transition-colors flex items-center justify-center gap-2"
          >
            <Navigation size={16} />
            Open in Google Maps
          </button>
